...
1. 本机：`https://localhost:5001/api/v2/health` 返回 `ok`。
...
普通 PowerShell（脚本会自动请求管理员权限/UAC）：
...
或者 PowerShell（脚本会自动请求管理员权限/UAC）：
...
```powershell
.\ops\ps\restart-windows-service.ps1
```
...
Invoke-RestMethod https://localhost:5001/api/v2/health
```
